  ...
  user_input_output_generation <- eventReactive(input$show_results, {
    ...
    if (input$country_or_area == "Whole country") {
      ...
      withProgress(message = "Preparing results, please wait", {
        ...
        adm2 <- sf::st_read(
          paste0("", get_iso3(country), "/shp/gadm41_", get_iso3(country), "_2.shp")
        )

        adm2$`Admin-2 unit` <- stri_trans_general(
          str = gsub("-", "_", adm2$NAME_2),
          id = "Latin-ASCII"
        )

        adm2$`Admin-2 unit` <- stri_trans_general(
          str = gsub(" ", "_", adm2$`Admin-2 unit`),
          id = "Latin-ASCII"
        )

        adm2$`Admin-1 unit` <- stri_trans_general(
          str = gsub("-", "_", adm2$NAME_1),
          id = "Latin-ASCII"
        )

        adm2$`Admin-1 unit` <- stri_trans_general(
          str = gsub(" ", "_", adm2$`Admin-1 unit`),
          id = "Latin-ASCII"
        )

        adm3 <- sf::st_read(
          paste0("", get_iso3(country), "/shp/gadm41_", get_iso3(country), "_3.shp")
        )

        adm3$`Admin-1 unit` <- stri_trans_general(
          str = gsub("-", "_", adm3$NAME_1),
          id = "Latin-ASCII"
        )

        adm3$`Admin-1 unit` <- stri_trans_general(
          str = gsub(" ", "_", adm3$`Admin-1 unit`),
          id = "Latin-ASCII"
        )

        adm3$`Admin-2 unit` <- stri_trans_general(
          str = gsub("-", "_", adm3$NAME_2),
          id = "Latin-ASCII"
        )

        adm3$`Admin-2 unit` <- stri_trans_general(
          str = gsub(" ", "_", adm3$`Admin-2 unit`),
          id = "Latin-ASCII"
        )

        adm3$`Admin-3 unit` <- stri_trans_general(
          str = gsub("-", "_", adm3$NAME_3),
          id = "Latin-ASCII"
        )

        adm3$`Admin-3 unit` <- stri_trans_general(
          str = gsub(" ", "_", adm3$`Admin-3 unit`),
          id = "Latin-ASCII"
        )

        adm3 <- adm3 %>% filter(`Admin-3 unit` %in% c("Seguela", "Bouafle", "Abengourou",
                                                      "Klouekanme-Toviklin-Lalo", "Zogbodomey-Bohicon-Zakpota",
                                                      "Bembereke-Sinende"))
        ...
        adm2_highlight <- adm2 %>%
          mutate(
            highlight_group = case_when(
              `Admin-2 unit` %in% c("Zogbodomey", "Bohicon", "Za_Kpota") ~ "Zogbodomey-Bohicon-Zakpota",
              `Admin-2 unit` %in% c("Bembereke", "Sinende") ~ "Bembereke-Sinende",
              `Admin-2 unit` %in% c("Klouekanme", "Toviklin", "Lalo") ~ "Klouekanme-Toviklin-Lalo",
              TRUE ~ NA_character_
            )
          ) %>%
          filter(!is.na(highlight_group))   # keep only highlighted groups


        final_ranked_table_full_EDITABLE <- final_ranked_table_full_EDITABLE %>% filter(!(`Region (admin-1)` %in% c("Atakora", "Alibori", "Nord_Ouest", "Extreme_Nord", "Gaza", "Maputo")))


        final_ranked_table_full_EDITABLE <- final_ranked_table_full_EDITABLE %>%
          mutate(
            `Rank of PMC option by cost-effectiveness` = row_number()
          )

        final_ranked_table_full_EDITABLE$Country <- country_lookup[final_ranked_table_full_EDITABLE$Country]
        final_ranked_table_full_EDITABLE <- final_ranked_table_full_EDITABLE %>% dplyr::select(-c(`Cost-effective? (lower country-specific threshold)`, `Cost-effective? (higher country-specific threshold)`, `Cost-effective? ($250 / DALY averted threshold)`)) %>%
          rename("Financial (budget) cost of SP+consumables" = "Financial cost of SP and administration consumables" ) %>%
          rename("Cumulative financial (budget) cost of SP+consumables" = "Cumulative cost of SP and administration consumables")  %>%
          relocate(`Rank of PMC option by cost-effectiveness`, .after = 2)
        ...
        writexl::write_xlsx(merged_df_annual_COMPLETE,
                            paste0(getwd(), "/merged_df_annual_COMPLETE_", country_code, "_", length(schedule), "_doses", percent_increase, ".xlsx"))

        writexl::write_xlsx(final_ranked_table_full_EDITABLE,
                            paste0(getwd(), "/final_ranked_table_", country_code, "_", length(schedule), "_doses", percent_increase, ".xlsx"))



        combined_df_with_PMC_reduction <- combined_df_with_PMC_reduction %>% filter(!(`Admin-1 unit` %in% c("Atakora", "Alibori", "Nord_Ouest", "Extreme_Nord", "Gaza", "Maputo")))

        ### MAP 1 — Cases averted
        clin_cases_averted <- ggplot(combined_df_with_PMC_reduction) +
          theme_bw() +
          theme(
          ...
          # choropleth FIRST
          geom_sf(aes(fill = clinical_cases_averted_with_PMC_per1000,
                      geometry = geometry),
                  size = 0) +

          scale_fill_viridis_c(
            name = "Cases averted per 1000",
            limits = c(0,200)
          ) +

          # admin boundaries AFTER fill
          geom_sf(data = adm2, fill = NA, color = "grey20", size = 0.5) +
          geom_sf(data = adm1, fill = NA, color = "grey40", size = 0.75) +

          # highlighted admin3 districts FOR CIV
          # geom_sf(
          #   data = adm3,
          #   aes(color = `Admin-3 unit`),
          #   fill = NA,
          #   size = 1.5
          # ) +
          #
          ...
          #     "Bouafle" = "pink",
          #     "Abengourou" = "orange",
          #     "Zogbodomey-Bohicon-Zakpota" = "#9933cc",
          #     "Bembereke-Sinende" = "pink",
          #     "Klouekanme-Toviklin-Lalo" = "orange"
          #   )
          # ) +
          ...
          geom_sf_label(
            aes(label = paste0(NAME_1), geometry = geometry),
            fun.geometry = st_centroid,
            size = 3,
            alpha = 0.2
          ) +

          ggtitle(paste0(length(schedule), " DOSE")) +
        ...
        ggsave(paste0(getwd(), "/clin_cases_averted_", country_code, "_", length(schedule), "_doses", percent_increase, ".png"), clin_cases_averted, width = 8, height = 8, dpi = 300)


        ### Prepare ICER ranking
        adm1_merged <- adm1_merged %>%
          filter(!(`Region (admin-1)` %in% c("Atakora", "Alibori", "Nord_Ouest", "Extreme_Nord", "Gaza", "Maputo"))) %>%
          arrange(`ICER based on regional costs only (no national costs)`) %>%
          mutate(`Rank of PMC option by cost-effectiveness` = row_number())


        ### MAP 2 — ICER + rank labels
        icer_rank_label <- ggplot(adm1_merged) +
          ...
          scale_fill_gradient2(
            ...
            mid = "#cefad0",
            high = "#ffffff",
            midpoint = round((3200+2)/2),
            limits = c(2, 3200),
            breaks = c(2, round((3200+2)/2), 3200),
            labels = c(
              paste0(comma(2), " (most cost-effective)"),
              comma(round((3200+2)/2)),
              paste0(comma(3200), " (less cost-effective)")
            ),
            guide = guide_colorbar(reverse = TRUE)
          ) +
        ...
        ggsave(paste0(getwd(), "/icer_rank_label_", country_code, "_", length(schedule), "_doses", percent_increase, ".png"), icer_rank_label, width = 8, height = 8, dpi = 300)

        ### MAP 3 — ICER + region labels
        icer_admin_label <- ggplot(adm1_merged) +
          ...
          geom_sf_label(
            aes(label = paste0(`Region (admin-1)`),
                geometry = geometry),
            fun.geometry = sf::st_point_on_surface,
            size = 3,
            alpha = 0.2
        ...
        ggsave(paste0(getwd(), "/icer_admin_label_", country_code, "_", length(schedule), "_doses", percent_increase, ".png"), icer_admin_label, width = 10, height = 10, dpi = 300)
        ...
        ggplot(combined_df_with_PMC_reduction) +
          theme_bw() +
          theme(
            legend.text=element_text(size=8),
            legend.title = element_text(size=10),
          ...
          geom_sf(data = adm2, fill = NA, color = "grey40", size = 0.5) +

          geom_sf(data = adm1, fill = NA, color = "black", size = 0.75) +
          ...
          scale_color_manual(
            ...
            values = c(
              ...
              "Bouafle" = "#377EB8",
              "Abengourou" = "#4DAF4A",
              "Zogbodomey-Bohicon-Zakpota" = "#E41A1C",
              "Bembereke-Sinende" = "#377EB8",
              "Klouekanme-Toviklin-Lalo" = "#4DAF4A"
            )
          ) +
        ...
        adm1_merged <- adm1_merged %>% filter(!(`Region (admin-1)` %in% c("Atakora", "Alibori", "Nord_Ouest", "Extreme_Nord"))) %>%
          arrange(`ICER based on regional costs only (no national costs)`) %>%
          mutate(
            `Rank of PMC option by cost-effectiveness` = row_number()
            )
        ...
        ggplot(adm1_merged) +
          ...
          geom_sf(data = adm1, color = "black", size = 0.75) +


          geom_sf(data = adm3, fill = NA, color = "grey70", size = 0.5) +

          geom_sf(
            aes(fill = `ICER based on regional costs only (no national costs)`,
                geometry = geometry),
            size = 0.5
          ) +

          # coloured district outlines
          geom_sf(
            data = adm3,
            aes(color = `Admin-3 unit`),
            fill = NA,
          ...
          geom_sf(data = adm2, fill = NA, color = "grey40", size = 0.2) +
          ...
          scale_fill_gradient2(
            ...
            mid = "#cefad0",
            high = "#ffffff",
            midpoint = round((644+80)/2),
            limits = c(80, 644),
            breaks = c(80, round((644+80)/2), 644),
            labels = c(
              paste0(comma(80), " (most cost-effective)"),
              comma(round((644+80)/2)),
              paste0(comma(644), " (least cost-effective)")
            ),
            guide = guide_colorbar(reverse = TRUE)
          ) +

          # health district legend
          scale_color_manual(
            name = "Health district",
            values = c(
              "Seguela" = "#E41A1C",
        ...
        ggplot(adm1_merged) +
          ...
          geom_sf(data = adm3, fill = NA, color = "grey70", size = 0.2) +

          geom_sf(
            data = subset(adm2, `Admin-2 unit` %in% c("Worodougou")),
            fill = NA,
            color = "black",
            size = 1.5
          ) +
          ...
          # coloured health district outlines
          geom_sf(
            data = adm3,
            aes(color = `Admin-3 unit`),
            fill = NA,
        ...
        # # Create flextable
        # ft <- flextable(merged_df_annual_COMPLETE)
        #
        # # Reduce font size
